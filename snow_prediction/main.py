...
def plot_actual(actual_series: list[TimeSeries]):
    # plot actual series
    plt.figure(figsize=figsize)
    labels = ['ADE2.csv', 'ALB2.csv']
...
def eval_model(pred_series: TimeSeries, actual_series: TimeSeries):
    # plot actual series
    plt.figure(figsize=figsize)
    actual_series[pred_series.start_time() - pd.Timedelta(12, unit='h'): pred_series.end_time()].plot(label="actual")
...
def validate(model: Model):
    start = model.train_target_transformed[0].end_time() + model.train_target_transformed[0].freq
    backtest_series = model.model.historical_forecasts(
        series=model.actual_target_transformed,
        start=start,
        num_samples=200,
...
def main():
    ...
    # model.fit()
    # model = Model(preproc, '1', 'TFT')
    # model.load('TFTModel_2024-12-09_17_43_16.pt')
    # model.transform()
    best_model = model.load_from_checkpoint()
    model.model = best_model
    model.transform()
    # plot_actual(model.actual_target_series)
    ...
    if model.model_name == "TFT":
        explainer = TFTExplainer(model.model, model.val_target_transformed, model.val_past_cov_transformed)
